...
- 共用的儲存庫中繼資料載入與已開始的審查者載入各以 30 秒為限；若背景回應遺失，35 秒的訊息等待保護機制會結束等待。等候最多 4 個並行請求名額的時間不計入期限。選用的審查請求事件查詢須在上層工作的剩餘期限內完成，總計最多 10 秒。逾時後仍會顯示相同的重新載入提示，並保留已載入的標籤；變更語言或顯示設定不會重新計時。
