...
[v1.18.2 版本說明](./docs/releases/v1.18.2.md)（英文）：優先透過 API 查詢發行狀態，重複使用已儲存商店資訊的驗證佐證，並分別提供套件重複使用與商店資訊更新的作業指引。擴充功能的審查者顯示行為維持不變。
...
- 一般瀏覽過程中，即使 GitHub 更新頁面，擴充功能仍會持續運作。若同一 PR 的 GitHub 原生中繼資料取代了擴充功能的顯示區域，擴充功能會將該區域還原。若載入審查者資訊期間 PR 列的原生中繼資料有所變更，目前的請求成功後會再次檢查該列；期間的多次變更會合併為一次後續檢查。
...
唯讀 `status` 不需建置或變更商店，即可產生已過濾敏感資訊的 JSON 報告與 Actions Summary，說明 API 狀態、已驗證的上傳紀錄及下一步發行作業。狀態查詢使用獨立佇列，不會取代等待中的發行作業。一般擴充功能套件發行時，若五種語言的說明文案與螢幕截圖未變，可重複使用已儲存商店資訊的既有驗證佐證，無須存取瀏覽器或反覆在後台儲存。商店資訊是否變更，只依實際提交的說明本文與依序排列的圖片判定，不包含說明區段外的貢獻者指引。報告分別列出套件重複使用狀態與尚待完成的商店資訊作業。套件尚未提交審查或發布時，商店資訊變更採用分階段程序；已有審查須結束後才能編輯商店資訊，已發布的套件則僅修改相關商店資訊，無須重新上傳。佐證缺漏或互相矛盾時，須核對相關內容。狀態報告僅記錄觀察結果，不代表授權；後續寫入作業一律重新執行受保護的驗證程序。
