...
唯讀 `status` 不需建置或變更商店，即可產生已過濾敏感資訊的 JSON 報告與 Actions Summary，說明 API 狀態、已驗證的上傳紀錄及下一步發行作業。一般擴充功能套件發行時，若五種語言的說明文案與螢幕截圖未變，可重複使用已儲存商店資訊的既有驗證佐證，無須存取瀏覽器或反覆在後台儲存。商店資訊有變更時，請使用分階段程序；佐證缺漏或互相矛盾時，須核對相關內容。狀態報告僅記錄觀察結果，不代表授權；後續寫入作業一律重新執行受保護的驗證程序。
