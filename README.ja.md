...
- 共有のリポジトリメタデータ取得と、開始済みのレビュアー情報取得は、それぞれ 30 秒に制限されます。バックグラウンドからの応答が届かない場合は、35 秒のメッセージ待機上限で終了します。同時に最大 4 件のリクエストを処理するための順番待ちは、この時間に含めません。補助的なレビュー依頼イベントの取得は、親処理の残り時間内で合計 10 秒までです。タイムアウト時も同じ再読み込みの案内を表示し、取得済みのチップを維持します。言語や表示設定を変更しても期限は延長されません。
