...
[v1.18.2 リリースノート](./docs/releases/v1.18.2.md)（英語）：API を優先するリリース状態の確認、保存済み掲載情報の検証証跡の再利用、パッケージ再利用と掲載情報更新の個別案内を追加しました。拡張機能のレビュアー表示の動作は変わりません。
...
- 通常の画面遷移で GitHub がページを更新しても動作を続けます。同じ PR の GitHub 標準メタデータが拡張機能の表示領域を置き換えた場合、その領域を復元します。レビュアー情報の取得中に PR 行の標準メタデータが変わった場合は、現在の取得が成功した後にその行を再確認します。取得中に複数回変わっても、後続の確認は一回にまとめます。
...
読み取り専用の `status` は、ビルドやストアの変更を行わず、機密情報を除いた JSON レポートと Actions Summary に API の状態、検証済みのアップロード記録、次のリリース作業を示します。独立したキューを使うため、待機中のリリースを置き換えません。通常のパッケージリリースでは、5 言語の説明文とスクリーンショットに変更がなければ、保存済み掲載情報の既存の検証証跡を再利用でき、ブラウザーへのアクセスやダッシュボードでの繰り返し保存は不要です。掲載情報の変更は、実際に提出する説明本文と順序付きの画像だけで判定し、説明区間外の貢献者向け案内は含めません。パッケージの再利用と残っている掲載情報の作業を別々に示します。パッケージが未提出・未公開なら掲載情報の変更には段階的な手順を使います。既に審査中なら審査終了後に掲載情報を編集し、公開済みパッケージでは再アップロードせず、対象の掲載情報だけを修正します。証跡の欠落や不整合がある場合は該当箇所を確認します。状態レポートは観察結果であり、承認の代わりにはなりません。以後の書き込みでは、必ず保護された検証手順を再実行します。
