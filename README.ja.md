...
読み取り専用の `status` は、ビルドやストアの変更を行わず、機密情報を除いた JSON レポートと Actions Summary に API の状態、検証済みのアップロード記録、次のリリース作業を示します。通常のパッケージリリースでは、5 言語の説明文とスクリーンショットに変更がなければ、保存済み掲載情報の既存の検証証跡を再利用でき、ブラウザーへのアクセスやダッシュボードでの繰り返し保存は不要です。掲載情報が変わる場合は段階的な手順を使い、証跡の欠落や不整合がある場合は該当箇所を確認します。状態レポートは観察結果であり、承認の代わりにはなりません。以後の書き込みでは、必ず保護された検証手順を再実行します。
