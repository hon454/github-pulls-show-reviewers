...
[v1.18.1 リリースノート](./docs/releases/v1.18.1.md)（英語）：従来のリポジトリ PR 一覧と Preview でのレビュアー表示に対応し、開発ツールの依存関係を整備しました。リリース準備時の全依存関係と本番依存関係の監査では、指摘事項はありませんでした。

## 主な機能

- 従来のリポジトリ PR 一覧と GitHub の New Repository Pull Requests Dashboard Preview の両方に対応し、ページ構造を自動で判別します。拡張機能側の設定は不要です。全体の PR ダッシュボード `github.com/pulls` はサポート対象外です。
...
- レビュー依頼中のユーザーに完了済みレビューもある場合、上限付きの issue event 取得で後の依頼を確認できたときだけ再依頼バッジを表示します。証拠が不完全または取得不能な場合は、依頼中の色と検索リンクを維持し、ツールチップとアクセシブルな名前に以前のレビューを残したまま、再依頼バッジを表示しません。
...
- 共有のリポジトリメタデータ取得と、開始済みのレビュアー情報取得は、それぞれ 30 秒に制限されます。バックグラウンドからの応答が届かない場合は、35 秒のメッセージ待機上限で終了します。同時に最大 4 件のリクエストを処理するための順番待ちは、この時間に含めません。補助的なレビュー依頼イベントの取得は、親処理の残り時間内で合計 10 秒までです。この補助的な取得だけがタイムアウトした場合、確認済みの再依頼の証拠を維持し、それ以外の曖昧な依頼は未確認のまま表示します。必須の取得がタイムアウトした場合は、同じ再読み込みの案内を表示し、取得済みのチップを維持します。言語や表示設定を変更しても期限は延長されません。
...
Node.js 22.12 以上と pnpm 10.x が必要です。WXT 0.21 では、`pnpm dev` は
ブラウザーを開かずに開発サーバーを起動します。`chrome://extensions` で
デベロッパーモードを有効にし、`.output/chrome-mv3-dev` をパッケージ化されていない
拡張機能として読み込み、開発サーバーを実行したままにしてください。任意のブラウザー
起動パッケージを除外する理由は、[依存関係の監査について](./CONTRIBUTING.md#dependency-audits)
（英語）を参照してください。
