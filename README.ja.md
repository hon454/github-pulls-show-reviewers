...
- **リポジトリアクセス:** サインインしたアカウントの非公開リポジトリが GitHub App インストールのアクセス範囲に含まれない場合、その所有者・リポジトリへの App アクセス設定を案内します。
- **組織:** 非公開の組織リポジトリを読み取るには、組織のオーナーによる GitHub App のインストールや承認が必要になる場合があります。
- **複数アカウント:** 個人用と仕事用のアカウントを同時に追加できます。拡張機能が各リポジトリに合うアカウントを選びます。
