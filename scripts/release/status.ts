...
export function statusSummary(report: StatusReport): string {
  ...
  return [
    ...
    `Async upload: ${report.remote.lastAsyncUploadState}; warned: ${report.remote.warned}; taken down: ${report.remote.takenDown}`,
    "Remote draft existence/version/ZIP hash: unknown. API version equality does not prove draft bytes.",
    `Listing baseline: ${report.listing.state}. Route: ${report.route}.`,
    "",
    ...report.receipts.map(
      (r) =>
