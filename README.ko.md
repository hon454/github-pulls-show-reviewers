...
[v1.18.2 릴리스 노트](./docs/releases/v1.18.2.md)(영어): API를 우선 활용하는 릴리스 상태 조회, 저장된 등록 정보 증빙 재사용, 패키지 재사용과 등록 정보 수정의 개별 안내를 추가했습니다. 확장 프로그램의 리뷰어 표시 동작은 그대로입니다.
...
- 요청된 리뷰어에게 완료된 리뷰도 있으면 제한된 이슈 이벤트 증거로 이후 요청이 확인될 때만 재요청 배지를 표시합니다. 증거가 불완전하거나 없으면 요청 색상과 검색 링크를 유지하고, 툴팁과 접근 가능한 이름에 이전 리뷰를 남기며, 재요청 배지는 숨깁니다.
...
- 일반적인 탐색 중 GitHub가 페이지를 갱신해도 계속 동작합니다. 같은 PR의 기본 메타데이터가 확장 프로그램의 표시 영역을 대체하면 해당 영역을 복구합니다. 리뷰어 정보를 불러오는 동안 PR 행의 기본 메타데이터가 바뀌면 현재 요청이 성공한 뒤 해당 행을 다시 확인합니다. 그동안 여러 번 바뀌어도 후속 확인은 한 번만 진행합니다.
...
- 공유 저장소 메타데이터 로드와 시작된 리뷰어 로드는 각각 30초로 제한하며, 백그라운드 응답이 유실되면 35초 메시지 안전장치가 대기를 끝냅니다. 최대 4개 요청 슬롯을 기다리는 시간은 기한에 포함하지 않습니다. 선택적 리뷰 요청 이벤트 조회는 상위 작업의 남은 기한 안에서 총 10초로 제한합니다. 이 선택적 조회만 시간 초과되면 이미 확인한 재요청 증거는 유지하고 나머지 모호한 요청은 확인되지 않은 상태로 표시합니다. 필수 요청이 시간 초과되면 같은 새로고침 안내를 표시하고 이미 불러온 칩을 유지합니다. 언어·표시 설정을 바꿔도 기한은 다시 시작되지 않습니다.
...
Node.js 22.12 이상과 pnpm 10.x가 필요합니다. WXT 0.21에서 `pnpm dev`는
브라우저를 열지 않고 개발 서버를 시작합니다. `chrome://extensions`에서 개발자
모드를 켜고 `.output/chrome-mv3-dev`를 압축 해제된 확장 프로그램으로 로드한 뒤
개발 서버를 계속 실행해 두세요. 선택적 브라우저 실행 패키지를 제외한 이유는
[의존성 감사 안내](./CONTRIBUTING.md#dependency-audits)(영어)를 참고하세요.
...
읽기 전용 `status`는 빌드나 스토어 변경 없이 API 상태, 검증된 업로드 기록, 다음 릴리스 작업을 담은 비밀 정보가 제거된 JSON 보고서와 Actions Summary를 생성합니다. 별도 큐를 사용하므로 대기 중인 릴리스를 대체하지 않습니다. 일반 패키지 릴리스에서는 5개 언어의 설명과 스크린샷이 그대로라면 저장된 등록 정보의 기존 검증 증빙을 재사용하므로, 브라우저 접속이나 대시보드의 반복 저장이 필요하지 않습니다. 등록 정보의 변경 여부는 실제 제출되는 설명 본문과 순서별 이미지로 판단하며, 설명 구간 밖의 기여자 안내는 포함하지 않습니다. 패키지 재사용 여부와 남은 등록 정보 작업은 별도로 표시합니다. 패키지가 아직 제출되거나 게시되지 않았다면 단계별 절차를 사용합니다. 이미 심사 중이면 심사가 끝난 뒤 등록 정보를 수정하고, 게시된 패키지는 재업로드 없이 필요한 등록 정보만 수정합니다. 증빙이 없거나 서로 맞지 않으면 해당 부분을 확인해야 합니다. 상태 보고서는 관찰 결과이며 승인을 대신하지 않습니다. 이후 쓰기 작업은 항상 보호된 검증 절차를 다시 수행합니다.
