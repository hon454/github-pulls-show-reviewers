...
읽기 전용 `status`는 빌드나 스토어 변경 없이 API 상태, 검증된 업로드 기록, 다음 릴리스 작업을 담은 비밀 정보가 제거된 JSON 보고서와 Actions Summary를 생성합니다. 일반 패키지 릴리스에서는 5개 언어의 설명과 스크린샷이 그대로라면 저장된 등록 정보의 기존 검증 증빙을 재사용하므로, 브라우저 접속이나 대시보드의 반복 저장이 필요하지 않습니다. 등록 정보가 바뀌면 단계별 절차를 사용하고, 증빙이 없거나 서로 맞지 않으면 해당 부분을 확인해야 합니다. 상태 보고서는 관찰 결과이며 승인을 대신하지 않습니다. 이후 쓰기 작업은 항상 보호된 검증 절차를 다시 수행합니다.
