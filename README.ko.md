...
- 공유 저장소 메타데이터 로드와 시작된 리뷰어 로드는 각각 30초로 제한하며, 백그라운드 응답이 유실되면 35초 메시지 안전장치가 대기를 끝냅니다. 최대 4개 요청 슬롯을 기다리는 시간은 기한에 포함하지 않습니다. 선택적 리뷰 요청 이벤트 조회는 상위 작업의 남은 기한 안에서 총 10초로 제한합니다. 시간 초과 시 같은 새로고침 안내를 표시하고 이미 불러온 칩을 유지합니다. 언어·표시 설정을 바꿔도 기한은 다시 시작되지 않습니다.
