...
只读 `status` 无需构建或更改商店，即可生成经过敏感信息过滤的 JSON 报告和 Actions Summary，说明 API 状态、已验证的上传记录及下一步发布操作。普通扩展包发布中，若五种语言的介绍文案和截图未变，可复用已保存商店信息的既有验证证据，无需浏览器访问或反复在后台保存。商店信息有变化时使用分阶段流程；证据缺失或冲突时，应核查相关内容。状态报告仅记录观察结果，不代表授权；后续写入操作始终重新执行受保护的验证流程。
