...
- Runs that resolve to a `v*` tag — whether triggered by a tag push or by
  `workflow_dispatch` with an existing tag — attach the zip to a GitHub Release
  and upload the same zip to the Chrome Web Store, submitting it with
  `DEFAULT_PUBLISH` so the update is published automatically after Chrome Web
...
- `release.yml` installs Playwright Chromium, re-runs `pnpm verify:release`, runs `pnpm zip`, creates the GitHub Release, then runs `pnpm cws:publish` whenever the run resolves to a `v*` tag (tag push or `workflow_dispatch` with an existing tag).
