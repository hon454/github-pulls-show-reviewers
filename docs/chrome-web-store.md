...
- Tag-triggered runs upload the zip through the Chrome Web Store API v2,
  submit it for review, and publish it automatically after approval.
- Manual runs expose a `chrome_web_store` choice: `skip`, credential-only
  `dry-run`, or `publish`.
...
workflow authentication logic changes. A dry run authenticates without
uploading a package or changing the store submission.
...
- A failure before `Publish to Chrome Web Store` does not change store state;
  fix the validation or packaging problem and rerun the workflow.
- A credential-only dry-run failure does not upload or submit anything.
- If the publish step fails, inspect both the job log and Developer Dashboard
  before rerunning. The package upload may have succeeded even when review
  submission failed, so do not blindly push another tag or cancel a pending
  review.
- If the dashboard already shows the version as a draft, under review, staged,
  or published, continue from that observed state. Cancel a submission only
  when intentionally replacing it.
...
6. Push the `v<version>` tag. The release workflow uploads the verified zip,
   submits it for review, and requests automatic publication after approval.
